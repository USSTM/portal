      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-base text-muted-foreground mt-1">
            Manage platform resources, links, and documents.
          </p>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead className="bg-secondary border-b border-border sticky top-0 z-10">
              <tr>
                ...
                <th className="bg-secondary px-6 py-4 text-xs font-medium text-muted-foreground uppercase tracking-wider text-center">
                  Order
                </th>
                <th className="bg-secondary px-6 py-4 text-xs font-medium text-muted-foreground uppercase tracking-wider">
                  Status
            ...
            <tbody className="divide-y divide-border relative">
              {resources.map((resource) => (
                <tr
                  className={`hover:bg-secondary/10 transition-colors group ${!resource.active ? 'opacity-75' : ''}`}
                  key={resource.id}
                >
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="text-xs text-muted-foreground hover:text-primary flex items-center gap-1 mt-1 truncate max-w-[250px]"
                      >
    ...
    <form className="space-y-3" onSubmit={onSubmit}>
      <div className="grid grid-cols-2 gap-3">
        <div className="col-span-2 sm:col-span-1 flex flex-col gap-1">
          ...
          <select
            defaultValue={resource?.category ?? 'finance'}
            name="category"
      ...
      <button
        className="w-full bg-secondary text-secondary-foreground hover:bg-secondary/80 px-2 py-1.5 rounded-md text-xs font-medium transition-colors flex items-center justify-center gap-1 mt-2"
        type="submit"
      >
        {resource ? (
          <>
            <Edit className="w-3 h-3" /> Save Changes
          </>
        ) : (
          <>
            <Plus className="w-3 h-3" /> Create Resource
          </>
        )}
      </button>
    </form>
