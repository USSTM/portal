...
import { z } from 'zod'
import { Search, Plus, Edit, MoreVertical, Archive, ArchiveRestore } from 'lucide-react'
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h2 className="text-3xl md:text-4xl font-bold text-foreground tracking-tight">Clubs</h2>
          <p className="text-base text-muted-foreground mt-1">Manage society clubs, affiliations, and status.</p>
        </div>
          ...
          <div className="absolute right-0 mt-2 z-50 w-[300px] sm:w-[450px] bg-card border border-border rounded-xl shadow-xl p-6">
            ...
            <form className="space-y-4" onSubmit={create}>
              <div className="grid grid-cols-2 gap-3">
                <input name="shortName" placeholder="Short name (e.g. CS Soc)" required className="col-span-2 sm:col-span-1 flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring" />
                <input name="fullName" placeholder="Full name" required className="col-span-2 sm:col-span-1 flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring" />
              </div>
              <input name="contactEmail" placeholder="Contact email (optional)" type="email" className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring" />
        ...
        <div className="overflow-x-auto pb-48 -mb-48">
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead className="bg-secondary/30 border-b border-border sticky top-0 z-10">
              <tr>
                <th className="px-6 py-4 text-xs font-medium text-muted-foreground uppercase tracking-wider">Club</th>
                <th className="px-6 py-4 text-xs font-medium text-muted-foreground uppercase tracking-wider">Short Name</th>
                <th className="px-6 py-4 text-xs font-medium text-muted-foreground uppercase tracking-wider">Contact</th>
            ...
            <tbody className="divide-y divide-border relative">
              {entries.map((club) => (
                <tr className={`hover:bg-secondary/10 transition-colors group ${club.lifecycle === 'archived' ? 'opacity-75' : ''}`} key={club.id}>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm text-foreground font-medium">{club.fullName}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-sm font-medium">{club.shortName}</td>
                  <td className="px-6 py-4 text-sm text-muted-foreground">
                    {club.contactEmail || <span className="italic opacity-50">None</span>}
                  </td>
                  <td className="px-6 py-4">
                    {club.lifecycle === 'active' ? (
                      ...
                      <span className="inline-flex items-center px-2 py-0.5 rounded-full bg-secondary text-muted-foreground text-xs font-medium border border-border">Archived</span>
                    )}
                  </td>
                  <td className="px-6 py-4 text-right">
                    <details className="relative inline-block text-left">
                      ...
                      <div className="absolute right-0 mt-2 z-50 w-[300px] bg-card border border-border rounded-xl shadow-xl p-4 text-left">
                        <h4 className="font-semibold text-sm mb-3 border-b border-border pb-2">Manage Club</h4>
                        ...
                        >
                          <input className="flex h-8 w-full rounded-md border border-input bg-transparent px-2 py-1 text-xs shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring" defaultValue={club.fullName} name="fullName" required placeholder="Full Name" />
                          <div className="flex gap-2">
                            <input className="flex h-8 w-1/3 rounded-md border border-input bg-transparent px-2 py-1 text-xs shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring" defaultValue={club.shortName} name="shortName" required placeholder="Short" />
                            <input className="flex h-8 w-2/3 rounded-md border border-input bg-transparent px-2 py-1 text-xs shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring" defaultValue={club.contactEmail ?? ''} name="contactEmail" type="email" placeholder="Contact Email" />
                          </div>
